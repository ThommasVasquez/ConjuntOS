...
CREATE INDEX IF NOT EXISTS encuestas_conjunto_idx ON encuestas (conjunto_id, created_at DESC);

-- One participation row per (survey, resident) → enforces one vote, even anonymous.
CREATE TABLE IF NOT EXISTS encuesta_participacion (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS encuesta_votos_idx ON encuesta_votos (encuesta_id, opcion_id);
