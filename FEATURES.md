...
_Verificado contra el código el 2026-06-12: 16 dominios backend, **85 endpoints REST**,
**65/65 tests pasando** (`cargo test --workspace`), frontend Next.js con 21 páginas._

Leyenda: ✅ implementado + probado · 🟡 implementado (sin tests dedicados) · ⚠️ parcial / diferido

---

## Backend (Rust / Axum) — 85 endpoints
...
## Frontend (Next.js 15 + Tailwind v4) — 21 páginas

✅ Tema **azul** (migrado desde morado), login, inicio (dashboard con Novedades),
perfil, pagos, parqueadero, mapa-parqueadero, reservas, visitantes, control-visitas,
clasificados, inmobiliaria, paquetería, citofonía, cartelera, pqrs, asamblea,
admin-finanzas, admin-novedades, admin-mensajes, admin-parqueadero, superadmin.
...
| Check | Resultado |
|---|---|
| `cargo test --workspace` | ✅ 65/65 |
| `cargo clippy -p enconjunto-migrate -- -D warnings` | ✅ 0 warnings |
| `cargo fmt --check` (migrate) | ✅ limpio |
| `--seed-demo` contra Postgres real | ✅ idempotente, 15 tablas pobladas |
| `next build` | ✅ 24 rutas |
