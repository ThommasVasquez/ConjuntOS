    ...
    <div className="flex flex-col gap-6 p-6 pt-16 pb-32 min-h-screen">
       ...
       <section className="fade-up liquid-glass rounded-3xl p-6 border border-border shadow-xl flex flex-col sm:flex-row gap-6 items-center justify-between">
          <div className="flex items-center gap-5">
             ...
             <div className="flex flex-col">
                ...
                <p className="text-xs text-text/70 leading-relaxed font-light">
                   {lastRound 
       ...
       <section className="fade-up flex flex-col sm:flex-row gap-4 items-center">
          <div className="relative flex-1 group w-full">
             <Search size={18} className="absolute left-5 top-1/2 -translate-y-1/2 text-text/50 group-focus-within:text-accent transition-colors" />
             <input 
               type="text" 
          ...
          <button className="w-14 h-14 rounded-2xl bg-primary-light/50 border border-border flex items-center justify-center text-text/60 hover:text-text transition-all active:scale-95 shadow-lg">
             <Filter size={20} />
          </button>
       </section>
       ...
       <section className="fade-up flex flex-col gap-4">
          <div className="flex justify-between items-center px-2">
             ...
             <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-text/5 border border-border">
                <Clock size={12} className="text-text/50" />
                <span className="text-[10px] text-text/65 font-bold uppercase tracking-widest">En Tiempo Real</span>
             </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
             {filteredRegistros.length === 0 && (
               <div className="liquid-glass rounded-3xl p-12 border border-dashed border-border text-center">
                  <p className="text-text/60 text-sm">No se encontraron registros que coincidan con la búsqueda.</p>
               </div>
             )}
             {filteredRegistros.map((reg, idx) => (
                <div key={idx} className="liquid-glass p-5 rounded-3xl border border-border/50 group hover:border-border transition-all shadow-xl">
                      ...
                      <div className="flex items-center gap-4">
                         ...
                         <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                               ...
                               <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-text/5 border border-border text-[9px] text-text/70 font-bold uppercase tracking-widest">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                               ...
                               <span className="text-[11px] text-text/60 font-medium">Hace {Math.floor((new Date().getTime() - new Date(reg.fecha).getTime())/60000)} min</span>
                            </div>
                         </div>
                      </div>
                   ...
                   {reg.observacion && (
                     <div className="mt-4 p-3 rounded-2xl bg-surface/50 border border-border text-xs italic text-text/65 leading-relaxed">
                        <span className="text-text/80 font-bold not-italic mr-1">Observación:</span> &quot;{reg.observacion}&quot;
                     </div>
                   )}
                </div>
             ))}
