      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <p className="text-text-secondary text-center text-sm mt-8">
            No hay mensajes aún. Escribe uno para contactar a tu {user?.rol === "HUESPED_TEMPORAL" ? "anfitrión" : "administración"}.
          </p>
        )}
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`flex ${msg.es_de_admin ? "justify-start" : "justify-end"}`}
          >
            <div
      ...
      <form onSubmit={handleSend} className="p-3 border-t border-border bg-surface-2">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-bg border border-border rounded-full px-4 py-2.5 text-sm text-text focus:outline-none focus:border-accent"
          />
          <button
            type="submit"
