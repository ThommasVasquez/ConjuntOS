...
const slides = [
  {
    ...
    description: "Eliminamos el desorden y la saturación en áreas comunes. Reserva piscinas, salones sociales y gimnasios de forma justa, transparente y sin filas desde tu celular.",
    image: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=1920&q=80",
    imageLight: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=1920&q=80",
    features: [
      {
  ...
  },
  {
    ...
    description: "Toma el control total de tu copropiedad. Cero desorden en recaudos, asambleas fluidas y transparencia absoluta en el uso de las instalaciones.",
    image: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=1920&q=80",
    imageLight: "https://images.unsplash.com/photo-1577495508048-b635879837f1?auto=format&fit=crop&w=1920&q=80",
    features: [
      {
  ...
  },
  {
    ...
    description: "Portería moderna sin filas ni congestiones. Autoriza visitantes y domicilios al instante con citofonía virtual y accesos QR verificados.",
    image: "https://images.unsplash.com/photo-1557597774-9d273605dfa9?auto=format&fit=crop&w=1920&q=80",
    imageLight: "https://images.unsplash.com/photo-1563906267088-b029e7101114?auto=format&fit=crop&w=1920&q=80",
    features: [
      {
  ...
  },
  {
    ...
    description: "Termina con el desorden vehicular. Asignación inteligente, control de tiempos de estancia y cero sorpresas en cupos de visitantes.",
    image: "https://images.unsplash.com/photo-1506521781263-d8422e82f27a?auto=format&fit=crop&w=1920&q=80",
    imageLight: "https://images.unsplash.com/photo-1573348722427-f1d6819fdf98?auto=format&fit=crop&w=1920&q=80",
    features: [
      {
...
export default function Hero() {
  ...
  const handleSlideChange = (index: number) => {
    ...
    gsap.timeline({ overwrite: "auto" })
      ...
      .to([".hero-text", ".hero-card"], { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" });
  };
      ...
      <div className={`relative w-full h-full overflow-hidden isolate ${isLight ? "bg-white" : "bg-[#000000]"}`}>
        ...
        <div className="relative z-20 w-full h-full flex items-center justify-between px-8 md:px-20">
          <div className={`hero-text max-w-2xl ${isLight ? "text-neutral-900" : "text-white"}`}>
            ...
            <h1 className={`text-4xl md:text-6xl lg:text-[4.2rem] font-medium leading-[1.1] mb-6 font-[family-name:var(--font-serif)] tracking-tight whitespace-pre-line ${isLight ? "" : "text-glow"}`}>
              {current.title}
            </h1>

            <p className={`text-base md:text-lg mb-10 max-w-lg font-[family-name:var(--font-inter)] leading-relaxed ${isLight ? "text-neutral-700" : "text-white/90"}`}>
              {current.description}
            </p>
          ...
          <div className={`hero-card backdrop-blur-xl p-10 rounded-[40px] w-[480px] shadow-2xl relative overflow-hidden border ${isLight ? "bg-white/80 border-black/10 shadow-[0_20px_60px_rgba(0,0,0,0.08)]" : "bg-white/5 border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.5)]"}`}>
            <div className="relative z-10">
              <div className="feature-content">
                ...
                <div>
                  <h3 className={`text-2xl font-medium font-[family-name:var(--font-serif)] mb-3 ${isLight ? "text-neutral-900" : "text-white"}`}>{currentFeature.title}</h3>
                  <p className={`text-sm leading-relaxed mb-6 h-16 ${isLight ? "text-neutral-700" : "text-white/80"}`}>{currentFeature.desc}</p>

                  <button className={`w-full py-3.5 rounded-full border text-xs font-bold tracking-widest uppercase hover:text-on-accent hover:bg-accent hover:border-accent transition-all duration-300 ${isLight ? "border-black/15 text-neutral-900" : "border-white/15 text-white"}`}>
                    Ver cómo funciona
                  </button>
                </div>
              </div>
