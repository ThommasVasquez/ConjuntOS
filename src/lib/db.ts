...
class ModelProxy {
      ...
      if (key === 'OR' && Array.isArray(val)) {
        ...
        Object.keys(val).forEach(op => {
          ...
          if (op === 'gte') currentQuery = currentQuery.gte(key, innerVal);
            ...
            const normalizedIn = Array.isArray(innerVal) 
              ? innerVal.map(item => item instanceof Date ? item.toISOString() : item)
              : [innerVal instanceof Date ? innerVal.toISOString() : innerVal];
            currentQuery = currentQuery.in(key, normalizedIn);
          }
        });
      } else {
  ...
  async create(args: any) {
    try {
      ...
      const tablesWithTimestamps = ["Reserva", "Tramite", "Inmueble", "Conjunto", "Usuario", "Notificacion", "Visita", "Vehiculo", "Mascota", "Parqueadero", "Novedad", "RondaParqueadero"];
