...
/**
 * Directory entry used by citofonia / intercom dialing. Includes the internal
 * number and role so the caller can dial and label the target.
 */
export interface DirectorioUsuarioDto {
  id: string;
  nombre: string;
...
/**
 * Native push subscription registration payload.
 * Sent to `POST /usuarios/me/push-subscriptions`.
 */
export interface NativePushSubscribeRequest {
