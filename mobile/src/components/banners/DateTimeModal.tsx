/**
 * Replacement for the web `<input type="datetime-local">` used by the
 * "Inicio" / "Fin" fields of the Banners Publicitarios form
 * (`src/app/(app)/admin-banners/page.tsx`). Year / month / day / hour / minute
 * are picked from plain lists — no date-picker dependency is added (see the
 * mobile port rules).
 *
 * It is a plain RN `Modal` (not the gorhom `Sheet`) because it is only ever
 * opened from inside another RN Modal (the banner form), and a gorhom sheet
 * cannot present over one. Same reasoning as `PickerModal` in
 * admin-novedades.tsx / `DateTimeModal` in the asamblea feature.
 *
 * Value contract: an ISO-8601 UTC string, exactly what `form.inicioEn` /
 * `form.finEn` hold on web (`new Date(input).toISOString()`). The web input is
 * `required` and always seeded, so there is no "clear" action here — an empty
...
export interface DateTimeModalProps {
  open: boolean;
  /** Current value as an ISO-8601 UTC string. */
  value: string;
...
export function DateTimeModal({ open, value, title, onClose, onChange }: DateTimeModalProps) {
  ...
  // Re-seed from the incoming ISO value each time the modal opens.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      <View className="flex-1 justify-end">
        {/* Inert backdrop: dismisses on tap but stays out of the accessibility
            tree, so the sheet content is reachable. */}
        <Pressable
          accessible={false}
          ...
          >
            ...
            >
              ...
              <View className="gap-2">
                <Text className="pl-1 text-[10px] font-bold uppercase tracking-widest text-text/60">
                  Año
                </Text>
              ...
              <View className="flex-row gap-3 pt-2">
                <Pressable
                  ...
                  className="flex-1 items-center justify-center rounded-2xl border border-border bg-surface2 py-3.5"
                >
                ...
                <Pressable
                  accessibilityRole="button"
                  onPress={() => {
                    onChange(new Date(year, month, safeDay, hour, minute, 0, 0).toISOString());
                    onClose();
                  }}
                  style={({ pressed }) => ({ opacity: pressed ? 0.85 : 1 })}
